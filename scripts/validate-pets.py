...
def validate_v2_sheet(path: Path, errors: list[str], details: dict[str, Any]) -> None:
    ...
    for row, used_count in enumerate(FRAME_COUNTS):
        for col in range(8):
            ...
            alpha_min, alpha_max = alpha_extrema(image.crop((left, top, left + CELL_SIZE[0], top + CELL_SIZE[1])))
            if col < used_count and alpha_max == 0:
                errors.append(f"blank required cell row {row}, column {col}")
            if col >= used_count and alpha_max != 0:
...
def validate_package(root: Path, entry: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        actual = {child.name for child in package_dir.iterdir() if child.is_file()}
